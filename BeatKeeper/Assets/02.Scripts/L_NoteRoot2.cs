...
public class L_NoteRoot2 : MonoBehaviour
{
    /// <summary>
    /// Animation Speed
    /// </summary>
    ...
    //draw
    protected ParabolaFly parabolaFly2;

    void OnDrawGizmos()
    {
        if (gizmo == null)
        {
            gizmo = new ParabolaFly(ParabolaRoot2.transform);
        }
    ...
    void Start()
    {
        ParabolaRoot2 = GameObject.Find("L_LnPos2");
        parabolaFly2 = new ParabolaFly(ParabolaRoot2.transform);
    ...
    void Update()
    {
        ...
        if (Animation && parabolaFly2 != null && animationTime < parabolaFly2.GetDuration())
        {
            int parabolaIndexBefore;
            int parabolaIndexAfter;
            parabolaFly2.GetParabolaIndexAtTime(animationTime, out parabolaIndexBefore);
            animationTime += Time.deltaTime;
            parabolaFly2.GetParabolaIndexAtTime(animationTime, out parabolaIndexAfter);

            transform.position = parabolaFly2.GetPositionAtTime(animationTime);
        ...
        else if (Animation && parabolaFly2 != null && animationTime > parabolaFly2.GetDuration())
        {
    ...
    public void FollowParabola()
    {
        RefreshTransforms(Speed);
        animationTime = 0f;
        transform.position = parabolaFly2.Points[0].position;
        Animation = true;
    ...
    {
        return parabolaFly2.getHighestPoint(parabolaIndex);
    }

    public Transform[] getPoints()
    {
        return parabolaFly2.Points;
    }

    public Vector3 GetPositionAtTime(float time)
    {
        return parabolaFly2.GetPositionAtTime(time);
    }

    public float GetDuration()
    {
        return parabolaFly2.GetDuration();
    }
    ...
    {
        parabolaFly2.RefreshTransforms(speed);
    }
    ...
    public static Vector3 ClosestPointInLine(Ray ray2, Vector3 point)
    {
        return ray2.origin + ray2.direction * Vector3.Dot(ray2.direction, point - ray2.origin);
    }
    ...
    public class ParabolaFly
    {
        ...
        public ParabolaFly(Transform ParabolaRoot2)
        {

            List<Component> components = new List<Component>(ParabolaRoot2.GetComponentsInChildren(typeof(Transform)));
            List<Transform> transforms = components.ConvertAll(c => (Transform)c);

            transforms.Remove(ParabolaRoot2.transform);
