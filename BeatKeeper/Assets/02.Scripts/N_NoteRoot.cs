...
public class N_NoteRoot : MonoBehaviour
{
    /// <summary>
    /// Animation Speed
    /// </summary>
    ...
    /// Start of Parabola
    /// </summary>
    public GameObject ParabolaRoot1;
    ...
    void OnDrawGizmos()
    {
        if (gizmo2 == null)
        {
            gizmo2 = new ParabolaFly(ParabolaRoot1.transform);
        }

        gizmo2.RefreshTransforms(1f);
        if ((gizmo2.Points.Length - 1) % 2 != 0)
            return;

        int accur = 50;
        Vector3 prevPos = gizmo2.Points[0].position;
        for (int c = 1; c <= accur; c++)
        {
            float currTime = c * gizmo2.GetDuration() / accur;
            Vector3 currPos = gizmo2.GetPositionAtTime(currTime);
    ...
    void Start()
    {
        ...
        parabolaFly = new ParabolaFly(ParabolaRoot1.transform);
